...
- the current excerpt from `draft.md`
...
2. selected shared style files referenced by `project.md`
...
- Prefer a local excerpt from `draft.md` over the whole document.
- Never load the entire `writings/styles/` folder. Load only the style files selected for the project.
- Read style choices from the `## Style Direction` section of `project.md`.
...
- research question
- selected resource passages
- any relevant excerpt from `draft.md`
...
- selected shared style files referenced by `project.md`
- selected claims, notes, or other working material when relevant
- target section or excerpt from `draft.md`
